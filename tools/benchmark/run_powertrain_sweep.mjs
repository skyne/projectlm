  ...
  },
  {
    ...
    build: {
      ...
      engine: {
        ...
        peak_torque_nm: 1080, peak_torque_rpm: 4200, base_vibration: 1.3,
  ...
  },
  {
    ...
    build: {
      ...
      engine: {
        ...
        peak_torque_nm: 560, peak_torque_rpm: 7800, base_vibration: 0.6,
        aspiration: "Single", drivetrain: "ParallelHybrid",
      },
    },
  },
  {
    ...
    build: {
      ...
      engine: {
        ...
        peak_torque_nm: 530, peak_torque_rpm: 7800, base_vibration: 0.6,
  ...
  },
  {
    ...
    build: {
      ...
      engine: {
        ...
        aspiration: "NA", drivetrain: "RangeExtender", generator_kw: 480,
      },
    },
  },
  {
...
function writeRaceConfig({ relPath, trackPath, carPath, sessionMode, durationHours, durationSeconds }) {
  ...
  // Deterministic dry weather — balance comparisons should not be dominated
  // by random red-flag/wet phases. Set WEATHER=random to restore variability.
  if (process.env.WEATHER !== "random") {
    // weather_* overrides are only honoured when weather_resolved=1.
    lines.push(
      "rng_seed=42",
      "weather_resolved=1",
...
async function main() {
  ...
  const families = familyFilter
    ...
    : FAMILIES;
  // TRIM=quick: 1 rear × 2 diffusers × 2 exhausts per family (fast validation).
  const quick = process.env.TRIM === "quick";
  const rears = quick ? REARS.slice(0, 1) : REARS;
  const diffusers = quick ? DIFFUSERS.slice(0, 2) : DIFFUSERS;
  const jobs = [];
  for (const fam of families) {
    const exhausts = quick ? fam.exhausts.slice(0, 2) : fam.exhausts;
