...
void InitWeatherState(WeatherState &weather, const std::string &profileId,
  ...
  const WeatherProfile profile = WeatherProfileForId(profileId);
...
void TickWeatherState(WeatherState &weather, const WeatherProfile &profile,
                      ...
                      std::mt19937 &rng) {
  weather.ambientTempC +=
      (profile.tempDriftPerHour / 3600.0) * deltaTime;

  const double rainRollChance =
      (profile.rainChancePerHour / 3600.0) * deltaTime;
  ...
  if (weather.phase == WeatherPhase::Dry || weather.phase == WeatherPhase::Cloudy) {
    ...
    } else if (weather.trackWetness < 0.05 && unit(rng) < rainRollChance * 0.5) {
      weather.forecastRainInSeconds =
          weather.forecastRainInSeconds < 0.0
              ? 120.0 + unit(rng) * 900.0
