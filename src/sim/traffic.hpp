...
struct TrafficModifiers {
  ...
  /** Fraction subtracted from throttle (chaser caution under blue flags). */
  double throttleLift = 0.0;
...
struct TrafficEvent {
  ...
  /** Set when otherEntryId was the car ahead under blue flag at contact. */
  bool defenderHadBlueFlag = false;
  /** True when the ahead car was lifting/cooperating under blue flag at contact. */
  bool defenderWasYielding = false;
  /** Legacy: whether ahead car had offline yield path (pit rejoin only now). */
  bool defenderOnYieldPath = false;
  /** Active blue-flag pass (faster car overtaking flagged slower car). */
  bool blueFlagPassActive = false;
  std::string blueFlagChaserId;
  std::string blueFlagDefenderId;
  /** Faster car had overtake path intent at contact. */
  bool chaserWasOvertaking = false;
  /** Slower car was defending/weaving instead of cooperating. */
