...
exports.pitLaneSpanM = pitLaneSpanM;
exports.generateDefaultPitLaneFields = generateDefaultPitLaneFields;
const SAMPLE_STEP_M = 12;
function vecLength(dx, dy, dz) {
...
function sampleTrackPoseAtLapDistance(points, closed, lapLength, lapDistance) {
...
function pitLaneSpanM(lapLength, entryT, exitT, closed) {
...
function generateDefaultPitLaneFields(track) {
    ...
    for (let i = 1; i < polyline.length; i++) {
        const seg = vecLength(polyline[i].x - polyline[i - 1].x, polyline[i].y - polyline[i - 1].y, polyline[i].z - polyline[i - 1].z);
        chordTotal += seg;
        if (i <= boxIdx)
            boxChord = chordTotal;
    }
