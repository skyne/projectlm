...
exports.weatherForEvent = weatherForEvent;
const TRACK_BIAS = {
...
function weatherForEvent(trackId, format, round) {
    const bias = TRACK_BIAS[trackId] ?? {};
    let profile = bias.profile ?? "dry";
    if (format === "24h" && trackId === "lemans_la_sarthe")
        profile = "changeable";
    if (format === "test" && trackId === "paul_ricard")
        profile = "hot_dry";
    if (round % 4 === 2)
        profile = "overcast";
    const ambientTempC = bias.ambientTempC ??
        (profile === "hot_dry" ? 30 : profile === "wet" ? 16 : 22);
    const trackWetness = bias.trackWetness ??
