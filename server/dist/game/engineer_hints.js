...
exports.EngineerHintManager = void 0;
exports.evaluateCarHint = evaluateCarHint;
const pit_planner_1 = require("./pitbot/pit_planner");
...
function tyreTreadFromSnap(s) {
    return (0, tyre_grip_1.normalizeTyreTread)(s.tireCompound);
}
function bodyNeedsHintRepair(s) {
    const ph = s.partHealth ?? {};
    for (const key of ["body_fl", "body_fr", "body_rl", "body_rr", "bodyFL", "bodyFR", "bodyRL", "bodyRR"]) {
        const health = ph[key];
        if (health != null && health < HINT_BODY_DAMAGE_THRESHOLD)
            return true;
    }
    return false;
}
function partWearIssue(s) {
    const ph = s.partHealth ?? {};
    let worstKey = null;
    let worst = 100;
    for (const [key, health] of Object.entries(ph)) {
        if (health == null || health >= PART_WEAR_THRESHOLD)
            continue;
...
function buildPitCommand(s, wet, opts) {
    const tread = wet >= tyre_grip_1.WET_TYRE_THRESHOLD ? "wet" : wet >= tyre_grip_1.INTER_TYRE_THRESHOLD ? "intermediate" : "slick";
...
function evaluateCarHint(snap, trackWetness) {
    const s = snap;
    if (s.retired || s.inPit || s.inGarage || s.pitQueued)
        return null;
    ...
    if ((0, pit_planner_1.needsEmergencyPit)(s)) {
        ...
        else if (fuelPct <= pit_planner_1.EMERGENCY_FUEL_FRACTION) {
        ...
        return {
            ...
            suggestedCommand: buildPitCommand(s, trackWetness, {
                fuel: fuelPct <= pit_planner_1.EMERGENCY_FUEL_FRACTION,
                tyres: flat.length > 0,
                repairs: (0, pit_planner_1.hasSevereCarIssue)(s) ? ["engine", "body"] : undefined,
            }),
        };
    }
    if (trackWetness >= tyre_grip_1.INTER_TYRE_THRESHOLD &&
        (0, tyre_grip_1.needsWeatherTyreSwap)(tread, trackWetness)) {
        const target = trackWetness >= tyre_grip_1.WET_TYRE_THRESHOLD ? "wet" : "intermediate";
        return {
            hintId: "",
            entryId: s.entryId,
            carNumber,
            category: "wrong_tyre",
            text: `Track is ${trackWetness >= tyre_grip_1.WET_TYRE_THRESHOLD ? "wet" : "damp"} — you're on ${tread} tyres. Box for ${target} rubber.`,
...
function pickMostUrgent(hints) {
    if (!hints.length)
        return null;
    return hints.sort((a, b) => CATEGORY_PRIORITY[a.category] - CATEGORY_PRIORITY[b.category])[0];
}
class EngineerHintManager {
    constructor() {
        this.activeHint = null;
        this.dismissedKeys = new Set();
        this.snoozeUntilRaceSec = new Map();
    ...
    dismiss(hintId) {
        if (!this.activeHint || this.activeHint.hintId !== hintId)
            return false;
     ...
     */
    tick(snapshots, managedEntryIds, trackWetness, raceTimeSec, timeScale, paused) {
        if (this.activeHint) {
...
exports.EngineerHintManager = EngineerHintManager;
