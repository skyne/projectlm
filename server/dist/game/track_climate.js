...
exports.TRACK_CLIMATE = void 0;
exports.climateForTrack = climateForTrack;
exports.monthName = monthName;
exports.buildClimateLabel = buildClimateLabel;
exports.resolveTrackWeather = resolveTrackWeather;
exports.formatWeatherConfigLines = formatWeatherConfigLines;
...
function climateForTrack(trackId) {
    return exports.TRACK_CLIMATE[trackId] ?? { ...DEFAULT_CLIMATE, trackId };
}
function monthName(month) {
    return MONTH_NAMES[clampMonth(month) - 1] ?? "June";
}
function rollSessionArchetype(rainWeight, rnd) {
    const wetChance = Math.min(0.15, 0.03 + rainWeight * 0.12);
    const changeableChance = Math.min(0.45, 0.1 + rainWeight * 0.32);
    const roll = rnd();
    if (roll < wetChance)
        return "wet";
    if (roll < wetChance + changeableChance)
        return "changeable";
    return "dry";
}
function biomeWeatherExtras(biome) {
    switch (biome) {
...
function applyArchetypeToProfile(profile, archetype) {
    if (archetype === "dry") {
        return {
            ...profile,
...
function buildClimateLabel(trackId, month, rainWeight, biome, archetype) {
    const place = (0, track_catalog_1.trackDisplayName)(trackId);
    const when = monthName(month);
    if (archetype === "dry") {
        if (biome === "arid")
            return `${when} at ${place} — desert dry`;
        return `${when} at ${place} — race day dry`;
    }
    if (archetype === "wet")
        return `${when} at ${place} — wet race likely`;
    if (biome === "arid")
        return `${when} at ${place} — mostly dry, rare showers`;
    if (rainWeight >= 0.75)
        return `${when} at ${place} — rain likely`;
    if (rainWeight >= 0.55)
        return `${when} at ${place} — changeable, showers possible`;
    if (rainWeight >= 0.35)
        return `${when} at ${place} — mixed skies`;
    return `${when} at ${place} — predominantly dry`;
}
function resolveTrackWeather(trackId, month, rngSeed = 0) {
    ...
    const rainChancePerHour = Math.min(0.55, lerp(0.008, 0.38, rainWeight) * (0.7 + climate.volatility * 0.3) * jitter);
    const baseWetness = Math.min(0.22, climate.baseWetnessBias + rainWeight * 0.1 + (rnd() - 0.5) * 0.015);
...
function formatWeatherConfigLines(resolved, rngSeed) {
