...
function buildClimateLabel(trackId, month, rainWeight, biome) {
...
function resolveTrackWeather(trackId, month, rngSeed = 0) {
    ...
    const rainChancePerHour = Math.min(0.92, lerp(0.02, 0.78, rainWeight) * (0.75 + climate.volatility * 0.35) * jitter);
    const baseWetness = Math.min(0.35, climate.baseWetnessBias + rainWeight * 0.18 + (rnd() - 0.5) * 0.02);
