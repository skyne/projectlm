...
exports.PERIMETER_SAMPLE_COUNT = exports.PERIMETER_BARRIER_WIDTH_M = exports.PERIMETER_GRASS_GAP_M = void 0;
exports.synthesizePerimeterSurfaces = synthesizePerimeterSurfaces;
/** Grass verge depth when no runoff/gravel is authored on that side. */
exports.PERIMETER_GRASS_GAP_M = 10;
/** Default synthesized barrier thickness (m). */
exports.PERIMETER_BARRIER_WIDTH_M = 1.2;
/** Samples around the lap when building continuous perimeter bands. */
exports.PERIMETER_SAMPLE_COUNT = 480;
const SYNTH_PREFIX = "synth:perimeter-";
function isBarrierSurface(surface) {
    return surface.startsWith("barrier");
}
function isSynthSegment(seg) {
    return seg.name?.startsWith(SYNTH_PREFIX) ?? false;
}
function sideMatches(segSide, side) {
    if (segSide === "both")
        return true;
    return segSide === side;
}
function halfWidthAtT(t, defaultWidthM, widthProfile) {
...
function segmentWidthAtT(seg, t) {
    ...
    if (env === "bell")
        return w0 + (w1 - w0) * Math.sin(u * Math.PI);
    return w0 + (w1 - w0) * u;
}
function segmentInnerEdgeM(halfW, seg, vergeWidthM) {
    const isKerb = seg.surface.startsWith("kerb");
    const verge = isKerb ? 0 : vergeWidthM;
    return halfW + (seg.innerOffsetM ?? 0) + verge;
}
function sampleSideState(t, side, authored, defaultWidthM, widthProfile, vergeWidthM) {
    const halfW = halfWidthAtT(t, defaultWidthM, widthProfile);
    let occupiedOuter = halfW;
    let explicitBarrier = null;
    for (const seg of authored) {
        if (t < seg.startT || t > seg.endT)
            continue;
        if (!sideMatches(seg.side, side))
            continue;
    ...
    let barrierInner;
    let barrierWidth = exports.PERIMETER_BARRIER_WIDTH_M;
    ...
    if (explicitBarrier) {
        ...
        barrierInner = halfW + exports.PERIMETER_GRASS_GAP_M;
        synthBarrier = true;
    }
...
function statesEqual(a, b, eps = 0.35) {
    return (Math.abs(a.grassInnerOffsetM - b.grassInnerOffsetM) < eps &&
...
function mergeSideSamples(samples) {
    if (!samples.length)
        return [];
    const merged = [];
...
function buildSideSegments(side, intervals, vergeWidthM) {
    const out = [];
...
function synthesizePerimeterSurfaces(input) {
    ...
    const sampleCount = Math.max(64, exports.PERIMETER_SAMPLE_COUNT);
    const synth = [];
    for (const side of ["outboard", "inboard"]) {
        ...
        for (let i = 0; i < sampleCount; i++) {
            ...
            samples.push({
                t,
                state: sampleSideState(t, side, authored, defaultWidthM, input.widthProfile, vergeWidthM),
            });
        }
