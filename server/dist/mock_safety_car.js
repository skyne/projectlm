...
exports.SAFETY_CAR_ENTRY_ID = void 0;
exports.createParkedMockSafetyCar = createParkedMockSafetyCar;
exports.deployMockSafetyCar = deployMockSafetyCar;
exports.peelOffMockSafetyCar = peelOffMockSafetyCar;
exports.parkMockSafetyCar = parkMockSafetyCar;
exports.tickMockSafetyCar = tickMockSafetyCar;
exports.buildMockSafetyCarSnapshot = buildMockSafetyCarSnapshot;
exports.SAFETY_CAR_ENTRY_ID = "safety-car";
...
function modLap(distance, lapLength) {
    if (lapLength <= 0)
        return 0;
...
function signedScAheadOfLeaderOnLap(leaderOnLap, scOnLap, lapLength) {
    leaderOnLap = modLap(leaderOnLap, lapLength);
    scOnLap = modLap(scOnLap, lapLength);
    if (scOnLap >= leaderOnLap)
        return scOnLap - leaderOnLap;
    const behind = leaderOnLap - scOnLap;
    if (behind < lapLength * 0.5)
        return -behind;
    return lapLength - behind;
}
function createParkedMockSafetyCar(lapLength) {
    return {
        phase: "parked",
        inPit: true,
...
function deployMockSafetyCar(sc, lapLength) {
    if (sc.phase === "on_track" || sc.phase === "exiting_pit")
        return;
...
function peelOffMockSafetyCar(sc) {
    if (sc.phase === "parked" || sc.phase === "entering_pit")
        return;
    sc.phase = "entering_pit";
}
function parkMockSafetyCar(sc, lapLength) {
    sc.phase = "parked";
    sc.inPit = true;
    sc.speed = 0;
    sc.pitLaneDistance = pitBoxDistanceM(lapLength);
}
function tickMockSafetyCar(sc, lapLength, frontPackRaceDistance, deltaTime) {
    if (deltaTime <= 0 || sc.phase === "parked")
        return;
    ...
    switch (sc.phase) {
        ...
        case "on_track": {
            ...
            sc.speed += Math.max(-SC_ACCEL_MS2 * deltaTime, Math.min(SC_ACCEL_MS2 * deltaTime, desired - sc.speed));
...
function sampleAtRaceDistance(distance, samples) {
...
function buildMockSafetyCarSnapshot(sc, lapLength, samples) {
    if (sc.phase === "parked")
        return null;
    const d = sc.inPit
        ? sc.pitLaneDistance
    ...
    return {
        entryId: exports.SAFETY_CAR_ENTRY_ID,
