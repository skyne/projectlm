...
export interface PitPlannerContext {
  ...
  /** Laps since the current tyre set was fitted (for true wear rate). */
  lapsOnTyres?: number;
}
...
function lapsUntilTyreWorn(s: PlannerSnap, lapsOnTyres?: number): number {
  const wear = s.tireWear ?? 0;
...
function reserveLapsForTank(
  lapsInTank: number,
  fullReserve: number,
  minReserve: number,
  headroomLaps: number,
): number {
  if (lapsInTank >= fullReserve + headroomLaps + 1) return fullReserve;
  return Math.max(minReserve, lapsInTank - headroomLaps);
}

export function burnScaledFuelBase(
  s: PlannerSnap,
  sincePit: number,
  fuelAtLastPit: number,
  ...
  const tank = tankCapacity(s);
  if (tank <= 0) return { low: profile.fuelLow, critical: profile.fuelCritical };
  // Lap 1 after a stop mixes pit-out delta with race burn — defer scaling.
  if (sincePit < 2) return { low: profile.fuelLow, critical: profile.fuelCritical };
  const burn = burnPerLap(s, sincePit, fuelAtLastPit);
  if (burn <= 0) return { low: profile.fuelLow, critical: profile.fuelCritical };

  const lapsInTank = tank / burn;
  const reserveLaps = reserveLapsForTank(
    lapsInTank,
    FUEL_RESERVE_LAPS_LOW,
    1.0,
    1.25,
  );
  const critReserveLaps = reserveLapsForTank(
    lapsInTank,
    FUEL_RESERVE_LAPS_CRITICAL,
    0.5,
    0.75,
  );
  ...
  const low = Math.min(
    0.75,
    Math.max(profile.fuelLow, (burn * reserveLaps) / tank),
  );
  const critical = Math.min(
    low * 0.8,
    Math.max(profile.fuelCritical, (burn * critReserveLaps) / tank),
  );
  return { low, critical };
}
...
export function planPitStop(
  ...
  const fuelSoon =
    lapsFuelLow > 0 && lapsFuelLow <= BUNDLE_LOOKAHEAD_LAPS;
