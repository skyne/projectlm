...
const FALLBACK_TANK: Record<string, number> = {
  Hypercar: 90,
  LMP2: 75,
  LMGT3: 120,
};
...
function profile(classId: string): ClassProfile {
  return PROFILES[classId] ?? PROFILES.Hypercar;
}

function tank(snap: CarSnapshot): number {
  return snap.fuelTankCapacity ?? FALLBACK_TANK[snap.classId] ?? 100;
}

function gapAheadSeconds(snap: CarSnapshot, snapshots: CarSnapshot[]): number {
...
function wetness(ctx: AiStrategyContext): number {
  return ctx.trackWetness ?? 0;
}

function wantsWetTyres(ctx: AiStrategyContext): boolean {
  const w = wetness(ctx);
  return w >= 0.42 || (ctx.rainIntensity ?? 0) >= 0.55;
}

function wantsDryTyres(ctx: AiStrategyContext): boolean {
...
function pickCompound(
  ...
  if (!changeTyres) return "";
  if (wantsWetTyres(ctx)) return "compound=wet|tires=all";
  if (wantsDryTyres(ctx) && snap.wetTyres) {
    return `compound=${(snap.tireCompound ?? "Medium").toLowerCase()}|tires=all`;
  }
  if (wantsDryTyres(ctx)) return "compound=medium|tires=all";
  const p = profile(snap.classId);
  if (p.preferTyreStops) return "compound=medium|tires=all";
  return "compound=medium|tires=all";
}

export function buildAiPitCommand(
  snap: CarSnapshot,
  snapshots: CarSnapshot[],
  ctx: AiStrategyContext,
): string | null {
  if (snap.retired || snap.inPit || snap.pitQueued) return null;
  if (snap.lap < 2 || !Number.isFinite(snap.fuel)) return null;
  ...
  const wrongTyres =
    (wantsWetTyres(ctx) && !snap.wetTyres) ||
  ...
  if (ctx.fcyActive || ctx.scActive) {
    if (!fuelCritical && !stint && !wrongTyres) return null;
  }

  if (!fuelCritical && !fuelLow && !tiresWorn && !repair && !stint && !wrongTyres)
    return null;
  if (
    (fuelLow || tiresWorn || repair || wrongTyres) &&
    !trafficWindow &&
    !fuelCritical &&
    !stint &&
    !wrongTyres
  ...
  const changeTyres =
    tiresWorn || stint || wrongTyres || (p.preferTyreStops && refuel && snap.tireWear > 0.45);
  ...
  if (repair) parts.push("repairs=engine");
  if (stint) parts.push("driver=swap");
  return parts.join("|");
}

export class AiStrategyManager {
  private context: AiStrategyContext = {
    raceTime: 0,
    targetDurationSeconds: 0,
  };

  setContext(ctx: Partial<AiStrategyContext>): void {
  ...
  tick(
    ...
    submit: (entryId: string, command: string) => boolean,
  ): void {
    for (const snap of snapshots) {
      if (snap.entryId === playerEntryId || snap.retired || snap.inPit) continue;
      const cmd = buildAiPitCommand(snap, snapshots, this.context);
      if (cmd) submit(snap.entryId, cmd);
