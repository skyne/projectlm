...
interface ClassStrategyProfile {
  fuelLowFraction: number;
  fuelCriticalFraction: number;
  tireWearThreshold: number;
  targetStintSeconds: number;
  minLapsBetweenStops: number;
}

const CLASS_PROFILES: Record<string, ClassStrategyProfile> = {
  Hypercar: {
...
const DEFAULT_PROFILE: ClassStrategyProfile = {
  fuelLowFraction: 0.28,
  fuelCriticalFraction: 0.12,
  tireWearThreshold: 0.74,
  targetStintSeconds: 2700,
...
const FALLBACK_TANK_LITERS: Record<string, number> = {
  Hypercar: 90,
  LMGT3: 120,
  LMP2: 75,
};
...
function profileFor(classId: string): ClassStrategyProfile {
  return CLASS_PROFILES[classId] ?? DEFAULT_PROFILE;
}

function tankLiters(snap: CarSnapshot): number {
...
function maxWheelWear(snap: CarSnapshot): number {
  const wheels = [
    snap.tireWearFL,
    snap.tireWearFR,
    snap.tireWearRL,
...
function pickCompound(
  stint: number,
  profile: ClassStrategyProfile,
  plan?: AiStintPlan,
): "soft" | "medium" | "hard" {
  if (plan?.compound) return plan.compound;
  if (stint === 0) return "medium";
  if (profile.targetStintSeconds <= 2200) {
    if (stint % 2 === 1) return "soft";
    return "hard";
  }
  if (stint % 3 === 1) return "soft";
  if (stint % 3 === 2) return "hard";
  return "medium";
}
...
function desiredDriverMode(
  snap: CarSnapshot,
  plan?: AiStintPlan,
): "push" | "normal" | "conserve" {
  ...
  if (plan?.driverMode) return plan.driverMode;
  return "normal";
}
...
function fuelLow(
  ...
  return (
    snap.fuel <= tank * profile.fuelCriticalFraction ||
    snap.fuel <= tank * lowFrac
  );
}

function fuelCritical(snap: CarSnapshot, profile: ClassStrategyProfile): boolean {
  const tank = tankLiters(snap);
  return snap.fuel <= tank * profile.fuelCriticalFraction;
}

function needsRegulatoryDriverSwap(snap: CarSnapshot): boolean {
  const rosterSize = snap.driverRoster?.length ?? 0;
  if (rosterSize < 2) return false;
  const maxStint = snap.maxDriverStintSeconds ?? 0;
  const stint = snap.driverStintSeconds ?? 0;
  if (maxStint > 0 && stint >= maxStint * 0.92) return true;
  const stamina = snap.driverStamina ?? 100;
  return stamina <= DRIVER_STAMINA_THRESHOLD;
}

function needsScheduledStop(
  snap: CarSnapshot,
  profile: ClassStrategyProfile,
  plan?: AiStintPlan,
): boolean {
...
export function evaluateAiPitStop(
  ...
  void ctx;
  if (snap.retired || snap.inPit || snap.pitQueued) return null;
  if (snap.lap < 2) return null;
  if (!Number.isFinite(snap.fuel) || snap.fuel < 0) return null;
  ...
  const planDriverSwap =
    plan?.driverChangeNextStop === true &&
    scheduledStop &&
  ...
  if (lowFuel || scheduledStop) reasons.push("fuel");
  if (changeTyres) reasons.push("tyres");
  if (needsDriver) reasons.push("driver");
  if (repairEngine) reasons.push("engine");
  if (scheduledStop && !lowFuel) reasons.push("stint");
  if (plan?.notes && scheduledStop) reasons.push("plan");
...
export class AiStrategyManager {
  private states = new Map<string, AiCarState>();

  reset(): void {
    this.states.clear();
  }

  private stateFor(entryId: string): AiCarState {
  ...
  tick(
    ...
    for (const snap of snapshots) {
      ...
      if (submitCommand(decision.entryId, decision.command)) {
        ...
        state.tireCompound = pickCompound(
          state.pitsCompleted,
          profileFor(snap.classId),
          getPlan?.(snap.entryId),
        );
