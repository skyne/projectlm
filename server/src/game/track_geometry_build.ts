...
import {
  ...
  type TrackSurfaceSegmentJson,
} from "./track_json";
import { synthesizePerimeterSurfaces } from "./perimeter_surfaces";

function parsePitLane(pit?: TrackPitLaneJson): TrackPitLaneGeometry | undefined {
  ...
  if (pit.entry_t != null) out.entryT = pit.entry_t;
  if (pit.exit_t != null) out.exitT = pit.exit_t;
  if (pit.box_distance_m != null) out.boxDistanceM = pit.box_distance_m;
  if (pit.speed_limit_ms != null) out.speedLimitMs = pit.speed_limit_ms;
...
function parseSurfaceProfile(
  profile?: TrackSurfaceSegmentJson[],
...
export function buildTrackGeometry(
  ...
  const polyline = canonicalPolyline(track).map((p) => ({ x: p.x, z: p.z }));
