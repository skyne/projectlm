...
type WeatherPhase = "Dry" | "Cloudy" | "LightRain" | "HeavyRain" | "Drying";
...
function weatherProfileForId(profileId: string): WeatherProfile {
...
function initWeatherPhase(trackWetness: number): WeatherPhase {
  if (trackWetness >= 0.55) return "HeavyRain";
  if (trackWetness >= 0.25) return "LightRain";
  if (trackWetness >= 0.08) return "Cloudy";
  return "Dry";
}
...
interface CarState {
  ...
  driverModeScale: number;
  pitServiceRemaining: number;
  pitFuelLiters: number;
  pitChangeTires: boolean;
  pitRepairEngine: boolean;
  pitDriverSwap: boolean;
...
const FUEL_BURN_PER_SEC: Record<string, number> = {
  Hypercar: 0.09,
  LMP2: 0.075,
  LMGT3: 0.055,
  solo: 0.07,
...
function isPitCommand(command: string): boolean {
...
function parsePitCommand(command: string): {
  ...
  for (const token of command.split("|")) {
    ...
    if (!part || part === "pit") continue;
    const eq = part.indexOf("=");
    if (eq === -1) continue;
    const key = part.slice(0, eq);
    const value = part.slice(eq + 1);
    if (key === "fuel") result.fuelLiters = Math.max(0, Number(value) || 0);
    else if (key === "tires" && value) result.changeTires = true;
    else if (key === "compound" && value) {
      result.changeTires = true;
      result.tireCompound = value;
    }
    else if (key === "repairs" && value.includes("engine")) result.repairEngine = true;
    else if (key === "driver" && value) result.driverSwap = true;
  }
  return result;
}

function serviceDuration(request: ReturnType<typeof parsePitCommand>): number {
  let duration = 2;
  if (request.fuelLiters > 0) duration += Math.min(8, request.fuelLiters * 0.04);
  if (request.changeTires) duration += 4;
  if (request.repairEngine) duration += 6;
  if (request.driverSwap) duration += 3;
  return duration;
}
...
export class MockSimSession {
  ...
  private replayLog: Array<{ timestamp: number; entryId: string; command: string }> = [];
  private trackWetness = 0;
  private ambientTempC = 22;
  private rainIntensity = 0;
  private trackGripEvolution = 1;
  private weatherPhase: WeatherPhase = "Dry";
  private forecastRainInSeconds = -1;
  private weatherProfile: WeatherProfile = weatherProfileForId("dry");
  private weatherProfileId = "dry";
  ...
  private processCommands(): void {
    for (const { entryId, command } of this.pendingCommands) {
      ...
      if (!car || car.retired || car.inPit) continue;

      if (command.toLowerCase().startsWith("driver_mode=")) {
        const mode = command.slice(command.indexOf("=") + 1).toLowerCase();
        if (mode === "conserve") car.driverModeScale = 0.82;
        else if (mode === "push") car.driverModeScale = 1.05;
        else car.driverModeScale = 1;
        continue;
      }

      if (!isPitCommand(command)) continue;
  ...
  private tickWeather(deltaTime: number): void {
    ...
    if (this.weatherPhase === "LightRain" || this.weatherPhase === "HeavyRain") {
      this.rainIntensity = Math.min(
        ...
        this.rainIntensity + profile.wetRatePerSecond * deltaTime * 4,
      );
      this.trackWetness = Math.min(
        1,
        this.trackWetness +
          profile.wetRatePerSecond * deltaTime * (1 + this.rainIntensity),
      );
      if (this.trackWetness >= 0.55) this.weatherPhase = "HeavyRain";
    } else if (
      ...
      this.rainIntensity = Math.max(
        0,
        this.rainIntensity - profile.dryRatePerSecond * deltaTime * 3,
      );
      this.trackWetness = Math.max(
        profile.baseWetness,
        this.trackWetness -
          profile.dryRatePerSecond * deltaTime * (1.2 + this.raceTime / 7200),
      );
      if (
        this.trackWetness <= profile.baseWetness + 0.03 &&
      ...
      this.trackWetness = Math.max(
        profile.baseWetness,
        this.trackWetness - profile.dryRatePerSecond * deltaTime,
      );
    }
    ...
    if (this.trackWetness >= 0.55) this.weatherPhase = "HeavyRain";
    else if (this.trackWetness >= 0.25) this.weatherPhase = "LightRain";
  ...
  private processPitServices(deltaTime: number): void {
    for (const car of this.cars) {
      if (!car.inPit || car.retired) continue;
      car.pitServiceRemaining = Math.max(0, car.pitServiceRemaining - deltaTime);
      if (car.pitServiceRemaining > 0) continue;

      if (car.pitFuelLiters > 0) {
        car.fuel = Math.min(car.fuelTankCapacity, car.fuel + car.pitFuelLiters);
      }
      if (car.pitChangeTires) car.tireWear = 0.05;
      if (car.pitRepairEngine) car.engineHealth = Math.min(100, car.engineHealth + 18);
  ...
  tick(deltaTime: number): void {
    ...
    for (const car of this.cars) {
      if (car.retired || car.inPit) continue;
      ...
      car.engineHealth = Math.max(
        0,
        car.engineHealth - deltaTime * 0.0025 * (1.1 - car.engineHealth / 100),
      );
